...
(function($, window, document, undefined)
{
    ...
    $.fn.contextMenu = function(method, selector, option)
    {
        ...
        {
            if (!((selector instanceof Array) || (typeof selector === 'string')
                || (selector.nodeType) || (selector.jquery)))
            {
                option = selector;
                selector = null;
            }
            ...
            this
                .each(function()
                {
                    ...
                    if (typeof selector === 'object')
                    {
                        ...
                        {
                            var name = selector[i].name, disable = selector[i].disable, elm = menu
                    ...
                    if (option)
                    {
                        if (eventType != option.triggerOn)
                        {
                            ...
                            //to bind event
                            trgr.bind(eventType + '.contextMenu',
                                iMethods.eventHandler);
                        }
                    }
                });
        },
        ...
        destroy : function()
        {
            this
                .each(function()
                {
                    ...
                    trgr.unbind('.contextMenu').removeClass('iw-mTrigger')
    ...
    var iMethods = {
        ...
        eventHandler : function(e)
        {
            ...
            } else if (option.displayAround == 'trigger')
            {
                var triggerHeight = trigger.outerHeight(true), triggerWidth = trigger
                    .outerWidth(true), triggerLeft = cntWin ? trigger.offset().left
                    - cObj.scrollLeft()
                    : trigger.offset().left - cLeft, triggerTop = cntWin ? trigger
                    .offset().top
                    - cObj.scrollTop()
                ...
                {
                    menuHeight = Math.min(menuData.menuHeight, (cHeight
                ...
                {
                    menuWidth = Math.min(menuData.menuWidth, (cWidth
            ...
            //to draw contextMenu
            var outerLeftRight = menu.outerWidth(true) - menu.width(), outerTopBottom = menu
                .outerHeight(true)
                - menu.height();
            ...
            var cssObj = {
                ...
                'height' : '',
                'width' : '',
                'overflow-y' : menuHeight != menuData.menuHeight ? 'auto'
                    : 'hidden',
                'overflow-x' : menuWidth != menuData.menuWidth ? 'auto'
                    : 'hidden'
            };
        ...
        keyEvent : function(e)
        {
            ...
            if (e.data.method == 'menu')
            {
                var curMenu = $('.iw-curMenu'), optList = curMenu
                    ...
                    .filter('.iw-mSelected'), index = optList.index(selected), focusOn = function(
                    elm)
                {
        ...
        createMenuList : function(trgr, selector, option)
        {
                ...
                && (!selector.jquery))
            {
                ...
                for (var i = 0; i < selector.length; i++)
                {
                    var selObj = selector[i], name = selObj.name, fun = selObj.fun, subMenu = selObj.subMenu, img = selObj.img
                        || '', title = selObj.title || "", className = selObj.className
                        || "", disable = selObj.disable, list = $('<li title="'
                        + title + '" class="' + className + '">' + name
                        + '</li>');
                    if (img)
                    {
                        list.prepend('<img src="' + img
        ...
        optionOtimizer : function(method, option)
        {
            ...
            if ($.inArray(option.triggerOn, [ 'hover', 'mouseenter',
                'mouseover', 'mouseleave', 'mouseout', 'focusin', 'focusout' ]) != -1)
            {
