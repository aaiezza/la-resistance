...
public class ResistanceController {
  ...
  @Autowired
  public ResistanceController(VoteCounter voteCounter, BoardFactory boardFactory) {
  ...
  public HashMap<String, String> vote(
      HttpSession session, @RequestHeader(required = true, value = "vote") boolean _vote) {
    if (!SESSION_TRACKER.contains(session)) SESSION_TRACKER.add(session);
