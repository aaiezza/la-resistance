...
public class Game extends MessageRelayer<Game> {
  ...
  private static final String MES_WINNER =
      "<span id='winner' class='%1$s'>The %1$sS are Victorious!</span>";

  private static final String MES_MISSION_SUCCESS =
      "<span id='successful'>Mission #%d was a Success! (%d : %d)</span>";

  private static final String MES_MISSION_FAILURE =
      "<span id='failure'>Mission #%d was a Failure! (%d : %d)</span>";

  private static final String MES_TEAM_APPROVE =
      "<table class='teamFull'>%s</table><span>%s's team is out on their Mission! (%d : %d)</span>";

  private static final String MES_TEAM_DENY =
      "<table class='teamFull'>%s</table><span id='failure'>The Resistance did NOT agree with %s's Team! (%d : %d)</span>";
  ...
  private synchronized boolean progressState() {
    switch (state) {
      ...
      case LEADER_CHOOSING_TEAM:
        if (!board.getCurrentMission().teamIsFull()) break;
      ...
      case RESISTANCE_VOTES_ON_TEAM:
        if (board.getTeamVoter().getResults().isPasses()) {
          ...
          message.add(
              String.format(
                  MES_TEAM_APPROVE,
                  board.getCurrentMission().getHTMLTeam(),
                  getCurrentLeader(),
                  board.getTeamVoter().getResults().approves(),
                  board.getTeamVoter().getResults().denies()));
        } else {
          String denyMessage =
              String.format(
                  MES_TEAM_DENY,
      ...
      case TEAM_VOTES_ON_MISSION:
        if (board.getCurrentMission().isSuccessful()) {
          message.add(
              String.format(
                  MES_MISSION_SUCCESS,
                  board.getCurrentMission().MissionNumber,
                  board.getCurrentMission().getMissionVotes().getResults().approves(),
                  board.getCurrentMission().getMissionVotes().getResults().denies()));
          successfulMissions++;
        } else {
          message.add(
              String.format(
  ...
  /**
   * The reason we want this method is because the impersonal {@link #broadcastGame} does not
   * contain roles. The {@link #broadcastGameToEachPlayer()} however, is a personal broadcast that
   * contains only that user's role so as to not reveal it to any other client or listener
   */
  ...
  public String getCurrentLeader() {
    if (state.equals(AWAITING_PLAYERS)
        || state.equals(PLAYERS_LEARNING_ROLES)
    ...
    if (saveThePlayers) {
      board
          ...
          .addAll(
              players.subList(
                  0,
                  players.size() >= board.getNumPlayers()
                      ? board.getNumPlayers()
                      : players.size()));
  ...
  boolean addPlayer(Player player) {
    if (!state.equals(AWAITING_PLAYERS) || board.getPlayers().contains(player) || board.isFull()) {
  ...
  public void addTeammate(String leader, String username) {
    if (!state.equals(LEADER_CHOOSING_TEAM)
        || !getPlayers().contains(username)
