...
public class AI extends Player {
  ...
  private boolean decideToLikeTeam() {
    if (activeGame.getDefaultScopeBoard().getCurrentLeader() == this) return true;

    if (activeGame.getTeamVoteTracker() == 4) return getRole() == LOYAL;

    List<Player> team = activeGame.getDefaultScopeBoard().getCurrentMission().getTeam();

    int dislikes =
        getRole() == SPY
            ? team.size() - activeGame.getDefaultScopeBoard().getCurrentMission().MinimumFails
            : activeGame.getDefaultScopeBoard().getCurrentMission().MinimumFails;

    for (Player p : team) {
      if (!likePlayer(p, null)) dislikes--;
  ...
  private boolean likePlayer(Player pick, final Stack<Player> picks) {
    // Arrogance Standard
    if (pick.equals(this)) return true;
    ...
    if (getRole() == SPY) {
      ...
      for (Mission m : activeGame.getMissions()) {
        ...
        if (!m.isSuccessful() && m.getTeam().contains(pick)) {
          choose =
              Math.random()
                  >= ((double) m.getMissionVotes().getResults().denies()) / ((double) m.TeamSize);
          break;
        }
      }
