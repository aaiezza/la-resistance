...
public class ShabaJdbcUserDetailsManager extends JdbcUserDetailsManager {
  ...
  public static final String QUERY_USER_BY_USERNAME =
      "SELECT users.username, password, enabled, first_name, last_name, email, date_joined, last_online, role FROM users LEFT JOIN user_role ON users.username=user_role.username WHERE users.username = ?";

  /** */
  public static final String NEW_USER_SQL =
      "INSERT INTO users (username, password, enabled, first_name, last_name, email) VALUES ( ?, ?, ?, ?, ?, ? )";

  /** */
  public static final String NEW_USER_ROLE_SQL =
  ...
  /** */
  public static final String DELETE_USER_ROLE_SQL = "DELETE FROM user_role WHERE username = ?";

  /** */
  public static final String SELECT_ALL_USERS_SQL =
      "SELECT users.username, password, enabled, first_name, last_name, email, date_joined, last_online, role FROM users LEFT JOIN user_role ON users.username=user_role.username";

  /** */
  public static final String SELECT_ALL_ROLES_SQL = "SELECT role FROM roles";

  /** */
  public static final String DELETE_USER_AUTHORITIES_SQL =
      "delete from user_role where username = ?";

  /** */
  public static final String UPDATE_USER_SQL =
      "UPDATE users SET enabled = ?, first_name = ?, last_name = ?, email = ? WHERE username = ?";

  /** */
  public static final String UPDATE_USER_LAST_ONLINE_SQL =
      "UPDATE users SET last_online = DATETIME('NOW', 'LOCALTIME') WHERE username = ?";
...
//  private ShabaJdbcUserDetailsManager(@Value("${newUserEnabled}") boolean newUserEnabled) {
  ...
  public void changePassword(String oldPassword, String newPassword)
    ...
    if (authenticationManager != null) {
      ...
      authenticationManager.authenticate(
          new UsernamePasswordAuthenticationToken(username, DigestUtils.sha1Hex(oldPassword)));
    } else {
    ...
    SecurityContextHolder.getContext()
        .setAuthentication(createNewAuthentication(currentUser, newPassword));
  ...
  public void changePassword(String username, String oldPassword, String newPassword)
    ...
    if (authenticationManager != null) {
      logger.debug(
          "Reauthenticating user '" + currentUser.getName() + "' for password change request.");

      authenticationManager.authenticate(
          new UsernamePasswordAuthenticationToken(currentUser.getName(), oldPassword));
    } else {
      logger.debug("No authentication manager set. Password won't be re-checked.");
    }
  ...
  public List<ShabaUser> getUsers() {
    List<ShabaUser> users =
        getJdbcTemplate().query(SELECT_ALL_USERS_SQL, new ShabaUserListExtractor());
  ...
  public ShabaUser getShabaUser() {
    ...
    if (auth != null && auth.getPrincipal() instanceof UserDetails) {
      shabaUser = loadShabaUserByUsername(((UserDetails) auth.getPrincipal()).getUsername());
    }
  ...
  protected ShabaUser createUserDetails(
      String username, ShabaUser userFromUserQuery, List<GrantedAuthority> combinedAuthorities) {
    ...
    return new ShabaUser(
        returnUsername,
        userFromUserQuery.getPassword(),
        userFromUserQuery.isEnabled(),
        userFromUserQuery.getFirst_name(),
        userFromUserQuery.getLast_name(),
        userFromUserQuery.getEmail(),
        userFromUserQuery.getDate_joined(),
        userFromUserQuery.getLast_online(),
        true,
        true,
        true,
        combinedAuthorities);
  ...
  protected List<ShabaUser> loadShabaUsersByUsername(String username) {
    return getJdbcTemplate()
        .query(QUERY_USER_BY_USERNAME, new String[] {username}, new ShabaUserMapper());
  }
  ...
  private class ShabaUserListExtractor implements ResultSetExtractor<List<ShabaUser>> {
    ...
    public List<ShabaUser> extractData(ResultSet rs) throws SQLException {
      HashMap<String, ShabaUser> results =
          (this.rowsExpected > 0
              ? new HashMap<String, ShabaUser>(rowsExpected)
      ...
      while (rs.next()) {
        ...
        if (results.containsKey(user.getUsername())) {
          ...
          results.put(
              user.getUsername(), createUserDetails(user.getUsername(), user, combinedAuthorities));
        } else {
